...
COMMIT = '82f1632c4be441ba212fccc7a3cd336911cba246'
TREE = '130269b980d0bd2bd507bc01862f21430ae36d3b'
SOURCE = '950f19045facacf5151c90661de1ca28d30658df'
SOURCE_TREE = '0c288a694a4c2cba04a7ae331eb3a376bdd4c811'
INVENTORY_SHA = '99869df257e3651516c1eed688c8627d43c961214ee6c7a2eb15cdfbc6a78597'
EXPECTED_FILES = 3974
EXPECTED_BYTES = 615463127
PRODUCTION_RUN = 35809170261
...
def validate_execution_binding():
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        ...
        require(receipt['format'] == 'revealline-metadata-pages-artifact.v1' and receipt['controllerCommit'] == COMMIT and receipt['controllerTree'] == TREE and receipt['currentVersion'] == 'v0.88.0' and receipt['gameSourceRevision'] == SOURCE and receipt['qualifiedSourceTree'] == SOURCE_TREE, 'Wrong frozen receipt source/controller')
        require(receipt['publishable'] is True and receipt['browserAdmissionsRequired'] is True and receipt['admittedArchives'] == 56 and receipt['historicalBridges'] == 131, 'Receipt is not actual production eligible')
        ...
        for name, field in [('publication.json', 'configurationSha256'), ('catalog.json', 'catalogSha256')]:
            ...
            if name == 'catalog.json':
                ...
                require(len(releases) == 132 and len({row['version'] for row in releases}) == 132 and sum(row['version'] == 'v0.88.0' for row in releases) == 1, 'Wrong complete catalog coverage')
        extraction = json.loads(archive.read('zip-receipt.json'))
        require(extraction['version'] == 'v0.88.0' and extraction['gameSourceRevision'] == SOURCE and extraction['distributionSha256'] == '8d7cc857a2eb2d31432b27220d31b6a1ed33990fb6158de1e8c3fdf5d756706f' and extraction['manifestSha256'] == 'd5e33d0320608e6745a2d3803c886984ef43b5afb1314ec9f897139ad66481b1' and extraction['crcAndHashesVerified'] is True, 'Frozen ZIP source/hash identity mismatch')
