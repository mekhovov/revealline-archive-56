"""Read-only, fail-closed intake of the actual Archive56 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
BASE = 'https://mekhovov.github.io/revealline-archive-56/'
REPO = 'mekhovov/revealline-archive-56'
TREE = '4443f3a017da924836de658ebd5dbbe9cf96dc21'
INVENTORY_SHA = '28dc81e2809efb48b83896e00f18aaf3a7878719ab339543dc1766862525a455'
LOCK_SHA = 'b392f58e88250258b760116e95d4312557d871511cea570c9a900893f0b0579e'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1088 and sum(r['bytes'] for r in rows) == 590907537, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive56-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive56-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.87.0.json']
